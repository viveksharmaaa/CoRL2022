...
#Change  vars, xf index in Wf(x) for each case
...
np = pyimport("numpy")
vars = npzread("/home/vivek/PycharmProjects/CoRL2022/Npz_files/TLPRA/TLPRA.npz")
num_dim_x = vars["arr_6"]
num_dim_control = vars["arr_7"]
effective_dim_start = vars["arr_8"]
effective_dim_end = vars["arr_9"]
traj = npzread("/home/vivek/PycharmProjects/CoRL2022/Npz_files/TLPRA/TLPRA_closed_pts.npz")

function Wf(x)
        xf=reshape(x[effective_dim_start+1:effective_dim_end],(1,effective_dim_end-effective_dim_start)) 
        xbot = reshape(x[1:num_dim_x - num_dim_control],(1,num_dim_x - num_dim_control)) #x[effective_dim_start+1:effective_dim_end-num_dim_control]
...
npzwrite("TLPRA_closed.npz",Dict("xd" => xd, "x" => xc, "RE" => RME))
print("DONE")
